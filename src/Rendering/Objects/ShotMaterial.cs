...
public class ShotMaterial
{
    private readonly int _program;
    private readonly Dictionary<string, int> _uniformLocations;
    
    public ShotMaterial(Dictionary<ShaderType, string> sources)
    {
    ...
    private int CompileShader(KeyValuePair<ShaderType, string> shaderSource)
    {
        ...
        if (success == 0)
        {
            ShotLogger.LogError("Error compiling shader of type {0}: {1}", shaderSource.Key, GL.GetShaderInfoLog(shaderId));
